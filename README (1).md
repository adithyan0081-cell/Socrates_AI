...
```mermaid
flowchart TD
    A([Boot sequence<br/>“Preparing unnecessary questions…”]) --> B[Landing screen<br/>user types a task]
    B --> C{Parse the task}
    C --> D[detectDomain<br/>9 regex-scored domains]
    C --> E[detectWho<br/>relationship extraction]
    D --> F{AI mode key entered?}
    E --> F
    F -->|Yes| G[Claude generates<br/>the next question]
    G --> H{JSON valid &<br/>not a repeat?}
    H -->|No / timeout| I[Offline engine<br/>tiered question pools]
    F -->|No| I
    H -->|Yes| J[Render question card<br/>+ 4 useless options]
    I --> J
    J --> K[User picks an answer]
    K --> L[Reaction line<br/>+ mascot mood flash<br/>+ transcript log]
    L --> M{Question 23 reached<br/>or “Mathi, report tha”?}
    M -->|No| N[Escalate tier<br/>accent colour drifts]
    N --> F
    M -->|Yes| O([Final report<br/>Task completion: 0%])
```

*Top-down flow of a full session. Note that no branch of this diagram ever reaches a state where the task is performed — every path loops back into asking another question, and the only exit is a report confirming nothing was done.*
...
- **Sachin Satheesan:** Visual design and the entire mascot system — the SVG hologram, its 22 CSS-variable-driven mood states, and the mapping from answers to reactions and resting expressions. Built the boot sequence, the tier-reactive accent colour drift, the instrument panel and final report screen, the magnetic-button and ambient dust effects, and the accessibility layer (keyboard 1–4 answering, aria-live regions, full `prefers-reduced-motion` path).
